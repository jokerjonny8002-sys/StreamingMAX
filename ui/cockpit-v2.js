...
async function refreshV2CommandBar() {
  try {
    ...
    setChipState(
      "v2Rx3Chip",
      studio.rx3 ? "online" : "offline",
      studio.rx3 ? "RX3 CONNECTED" : "RX3 OFFLINE"
    );

    setChipState(
      "v2CameraChip",
      studio.camera ? "online" : "offline",
      studio.camera
        ? (studio.sonyCamera ? "SONY CAMERA CONNECTED" : "CAMERA CONNECTED")
        : "CAMERA OFFLINE"
    );

    setChipState(
      "v2ScarlettChip",
      studio.focusrite ? "online" : "offline",
      studio.focusrite ? "FOCUSRITE CONNECTED" : "FOCUSRITE OFFLINE"
    );
  } catch (error) {
    console.error(
...
function initializeMissionCommandNavigation() {
  const tabs =
    document.querySelectorAll("[data-command-module]");

  const views =
    document.querySelectorAll("[data-command-view]");

  if (!tabs.length || !views.length) return;

  const savedModule =
    localStorage.getItem("streamingMaxMissionCommandModule") ||
    "commander";
  ...
  function activateModule(moduleName) {
    tabs.forEach(tab => {
      tab.classList.toggle(
        "active",
        tab.dataset.commandModule === moduleName
      );
    });
    ...
    views.forEach(view => {
      view.classList.toggle(
        "active",
        view.dataset.commandView === moduleName
      );
    });

    localStorage.setItem(
      "streamingMaxMissionCommandModule",
      moduleName
    );
  }
  ...
  tabs.forEach(tab => {
    tab.addEventListener("click", () => {
      activateModule(tab.dataset.commandModule);
    });
  });
  ...
  const moduleExists =
    [...views].some(
      view =>
        view.dataset.commandView === savedModule
    );
...
document.addEventListener(
  "DOMContentLoaded",
  initializeMissionCommandNavigation
);
...
async function refreshHomeV3Mission() {
  try {
    ...
    if (
      typeof window.streamingMax.getAtlasMessage
        === "function"
    ) {
      const atlasMessage =
        await window.streamingMax.getAtlasMessage();

      const greeting =
        atlasMessage?.greeting ||
...
async function searchV2StudioEquipment() {
  ...
  try {
    ...
    if (results) {
      results.innerHTML = `
        <p class="studio-empty-v2">
          Equipment search failed: ${escapeStudioHtml(
...
async function initializeV2StudioBuilder() {
  ...
  /*
   * Prevent duplicate listeners if the department
...
async function runHomeSpeedTest(button) {
  ...
  try {
    ...
    const rating =
      result?.rating || "COMPLETE";

    if (status) {
      status.textContent =
        `${upload} Mbps UP • ${rating}`;
    }
...
let homeMissionTileTimer = null;

function setMissionTileState(
  button,
  status,
  active
) {
...
async function refreshHomeMissionTiles() {
  try {
    const [studio, sleepGuard] =
      await Promise.all([
        window.streamingMax.detectStudio(),
        window.streamingMax.getSleepGuard()
      ]);

    const obsButton =
      document.querySelector(
        '[data-main-launch="obs"]'
      );

    const rekordboxButton =
      document.querySelector(
        '[data-main-launch="rekordbox"]'
      );

    const sleepButton =
      document.getElementById(
        "homeSleepGuardButton"
      );
    ...
    const sleepStatus =
      document.getElementById(
        "homeSleepGuardStatus"
      );

    if (!obsButton?.classList.contains("working")) {
    ...
    if (
      !rekordboxButton?.classList.contains(
        "working"
      )
    ) {
      rekordboxStatus.textContent =
        studio.rekordbox
          ? "READY TO MIX"
          : "OFFLINE";
    ...
    if (!sleepButton?.classList.contains("working")) {
    ...
    console.error(
      "Mission tile refresh failed:",
      error
    );
  }
}

async function toggleHomeSleepGuard(button) {
  const status =
    document.getElementById(
      "homeSleepGuardStatus"
    );
  ...
  try {
    ...
    setHomeActionStatus(result.message);
    addHomeTimelineEvent(result.message);
...
function initializeHomeMainActions() {
  ...
  if (homeMissionTileTimer) {
    clearInterval(homeMissionTileTimer);
  }

  homeMissionTileTimer = setInterval(
    refreshHomeMissionTiles,
    4000
  );
}
...
function renderPerformanceProcesses(processes = []) {
  ...
  list.innerHTML = processes
    ...
    .map(process => {
      const level =
        process.level || "caution";

      const isProtected =
        Boolean(process.protected) ||
        level === "protected";

      return `
        <article
          class="performance-process-row process-${level}"
        >
          <div class="performance-process-identity">
            <div class="process-name-row">
              <span
                class="process-safety-light ${level}"
                aria-hidden="true"
              ></span>
            ...
            <p>
              PID ${escapeStudioHtml(
                process.pid || "--"
              )}
            </p>

            <span class="process-level ${level}">
              ${labels[level] || labels.caution}
            </span>
          </div>
          ...
          <button
            ...
            type="button"
            data-end-process="${escapeStudioHtml(
              process.pid
            )}"
            data-process-name="${escapeStudioHtml(
              process.name || "Unknown"
            )}"
            ${isProtected ? "disabled" : ""}
          >
            ${isProtected ? "LOCKED" : "END"}
          </button>
        </article>
      `;
    })
    ...
    .forEach(button => {
      button.addEventListener(
        ...
        async () => {
          ...
          const warningText = caution
            ? "\\n\\nATLAS marked this process USE CAUTION. Closing it may interrupt another application or macOS feature."
            : "\\n\\nUnsaved work in that application may be lost.";

          const confirmed = window.confirm(
            `End ${processName}?${warningText}`
          );
...
function setNetworkPlatformStatus(
  id,
  ready
) {
  const element =
...
function getNetworkAtlasMessage(result) {
  const upload =
    Number(result.uploadMbps || 0);
  ...
  if (result.rating === "EXCELLENT") {
    return {
      headline:
        "Your connection is mission ready.",
      message:
        `ATLAS recommends up to ${result.recommendedBitrate} kbps while preserving upload headroom.`
    };
  }
  ...
  if (result.rating === "GOOD") {
    return {
      headline:
        "Your connection is ready to stream.",
      message:
        `Upload is ${upload} Mbps with ${latency} ms latency. Use the recommended ${result.recommendedBitrate} kbps ceiling.`
    };
  }

  if (result.rating === "FAIR") {
    return {
      headline:
        "Streaming is possible with caution.",
      message:
        "ATLAS recommends lowering resolution or bitrate and closing bandwidth-heavy applications."
    };
  }
  ...
  return {
    ...
    message:
      "ATLAS recommends checking Wi-Fi strength, restarting the router, or using Ethernet before going live."
  };
}

function renderNetworkSpeedTest(result) {
  setNetworkText(
  ...
  setNetworkText(
    "networkResponsivenessValue",
    `${result.responsiveness} RPM`
  );
  ...
  setNetworkText(
    "networkBitrateValue",
    `${result.recommendedBitrate} kbps`
  );

  setNetworkPlatformStatus(
    "networkTwitchValue",
    Boolean(result.twitchReady)
  );

  setNetworkPlatformStatus(
    "networkTiktokValue",
    Boolean(result.tiktokReady)
  );

  setNetworkPlatformStatus(
    "networkYoutubeValue",
    Boolean(result.youtubeReady)
  );

  const report =
    getNetworkAtlasMessage(result);

  setNetworkText(
    "networkAtlasHeadline",
    report.headline
  );
  ...
  setNetworkText(
    "networkTestMessage",
    `Test complete on ${result.interfaceName || "your active connection"}.`
  );

  const badge =
    document.getElementById(
      "networkDepartmentStatus"
    );
...
async function runNetworkDepartmentSpeedTest(
  button
) {
  const originalText =
    button.textContent;
  ...
  setNetworkText(
    "networkTestMessage",
    "ATLAS is testing the connection. This normally takes about one minute."
  );
  ...
  try {
    ...
    renderNetworkSpeedTest(result);

    const summary =
      `Speed test: ${result.downloadMbps} Mbps down, ${result.uploadMbps} Mbps up, ${result.latencyMs} ms latency.`;
    ...
    console.error(
      "Network department speed test failed:",
      error
    );
...
function initializeNetworkDepartment() {
  const button =
    document.getElementById(
      "networkSpeedTestButton"
    );

  button?.addEventListener(
    "click",
    () => runNetworkDepartmentSpeedTest(
      button
    )
  );
}
...
function initializeCommanderProfile() {

    const config = window.StreamingMaxConfig;

    if (!config) {
        console.error("StreamingMaxConfig not found.");
        return;
    }

    const nameInput = document.getElementById("commanderNameInput");
    const callsignInput = document.getElementById("commanderCallsignInput");
    const greetingInput = document.getElementById("commanderGreetingInput");
    const missionSelect = document.getElementById("commanderMissionSelect");
    const startupGreeting = document.getElementById("commanderStartupGreeting");
    const rememberDepartment = document.getElementById("commanderRememberDepartment");
    const saveButton = document.getElementById("saveCommanderProfileButton");
    const status = document.getElementById("commanderSaveState");
    ...
    // EVERYTHING BELOW BELONGS INSIDE THE FUNCTION

        // Load previously saved Commander profile
       // Load Commander profile from the ATLAS shared state
    const atlasState = window.AtlasState;

    if (!atlasState) {
        console.error(
            "Commander Profile could not initialize because AtlasState is unavailable."
        );

        status.textContent = "ATLAS Core Unavailable";
        return;
    }

    const savedState = atlasState.getState();

    nameInput.value =
        savedState.commander?.name || "";

    callsignInput.value =
        savedState.commander?.callsign || "";

    greetingInput.value =
        savedState.commander?.greeting || "";

    missionSelect.value =
        savedState.mission || "dj-performance";

    startupGreeting.checked =
        savedState.startupGreeting ?? true;

    rememberDepartment.checked =
        savedState.rememberDepartment ?? true;

    console.log(
        "Commander Profile loaded from ATLAS State.",
        savedState
    );
    ...
    const commanderData = {
        ...
        startupGreeting: startupGreeting.checked,
        rememberDepartment: rememberDepartment.checked
    };

    console.log(
        "Sending Commander Profile to ATLAS...",
        commanderData
    );

    const saved =
        atlasState.updateCommander(commanderData);

    if (!saved) {
        status.textContent =
            "Unable to Save Mission Parameters";
    ...
    profile.nickname =
        commanderData.callsign;

    profile.preferredNameType =
        "realName";
    ...
    status.textContent =
        "Mission Parameters Saved ✔";

    console.log(
        "Commander Profile connected to ATLAS.",
        atlasState.getState()
    );
});
